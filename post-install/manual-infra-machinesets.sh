...
MACHINESET_POLICY_DIR="$CLUSTER_POLICY_DIR/machineset"

ls -d $MACHINESET_POLICY_DIR
if [ $? -ne 0 ]; then
   exit 10
fi

find $MACHINESET_POLICY_DIR  -type f | egrep -ie "\.yaml" -ie "\.yml" | grep -v "kustomization\.y" | while read yaml_file 
do
